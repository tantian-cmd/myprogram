...
# 使用训练的模型进行预测，为了方便，
# 案例直接把测试集里面的数据拿出来三条
x1=[13.52,3.17,2.72,23.5,97]#不使用测试集，使用实验报告中给定的5维指标
y1=[12.42,2.55,2.27,22,90]
z1=[13.76,1.53,2.7,19.5,132]
r=[x1,y1,z1]
print(r)#利用3个数据进行测试，即取3个数据作为模型的输入端
model.predict(r)
print(model.predict(r))#输出测试的结果，即输出模型预测的结果
